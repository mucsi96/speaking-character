...
ELEVENLABS_API_KEY=$(az keyvault secret show --vault-name "$AZURE_KEYVAULT_NAME" --name eleven-labs-api-key --query value -o tsv)
...
helm upgrade $RELEASE_NAME mucsi96/node-app \
    ...
    --set persistentVolumeClaims[0].name=party-pvc \
